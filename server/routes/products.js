...
router.get('/', verifyToken, async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT 
                p.id_producto, p.nombre, p.descripcion, p.precio, p.costo,
...
router.get('/public', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            ...
            GROUP BY p.id_producto, p.nombre, p.descripcion, p.precio, p.estado, c.nombre
            ORDER BY p.fecha DESC
        `);
        res.json({ ok: true, products: rows });
    } catch (error) {
...
router.post('/', verifyToken, async (req, res) => {
    ...
    const margen = costoNum ? parseFloat((precioNum - costoNum).toFixed(2)) : 0;
    ...
    try {
        ...
        const [result] = await conn.query(
            `INSERT INTO producto (nombre, descripcion, id_categoria, precio, costo, margen, estado, fecha)
             VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
            [nombre, descripcion || '', id_categoria, precioNum, costoNum, margen, estado || 'Activo']
        );
    ...
    try {
        ...
        await conn.query(
            `UPDATE producto 
             ...
             WHERE id_producto=?`,
            [nombre, descripcion || '', id_categoria, precioNum, costoNum, margen, estado || 'Activo', id]
        );
